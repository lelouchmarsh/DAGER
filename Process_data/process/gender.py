...
# Spliting the datas set into train and test set
...
output_dir_train_male = 'D:\\Gender data\\train\\male'
output_dir_train_female = 'D:\\Gender data\\train\\female'
...
output_dir_test_male = 'D:\\Gender data\\test\\male'
output_dir_test_female = 'D:\\Gender data\\test\\female'
...
for image in D_train.values:
    ...
    if image[0] == 'male':
        cv2.imwrite('D:\\Gender data\\train\\male' + str(counter) + '.jpg', img)
    else:
        cv2.imwrite('D:"\\Gender data\\train\\female' + str(counter) + '.jpg', img)
...
for image in D_test.values:
    ...
    # img = cv2.resize(img, (128,128))
    if image[0] == 'male':
        cv2.imwrite('D:\\Gender data\\test\\male' + str(counter) + '.jpg', img)
    else:
        cv2.imwrite('D:\\Gender data\\test\\female' + str(counter) + '.jpg', img)
    print('--('+str(counter)+')Processing--')
        counter += 1
