...
def label (crop_img):
	...
	# label = gender_list[[np.where(predict[0] == np.amax(predict[0]))]]
	if predict[0][0] > predict[0][1] :
		label = "female"
	if predict[0][1] > predict[0][0] :
		label = "male"
	return (label)
####### Xử lý ảnh và trả về label gender #######
...
labeled (path)	
# labeled ("F:/Project_Gender/test/male/2.jpg")
